...
def add_tag(r: Request, nome: str = Form(...), colore: str = Form("#0d6efd"), descrizione: str = Form(None)):
    ...
    if not nome_clean:
        return RedirectResponse(url=f"/admin/automezzi/tag?error={urllib.parse.quote('Il nome del tag non può essere vuoto.')}", status_code=303)
    ...
    try:
        with engine.begin() as conn:
            ...
            if exists:
                ...
                return RedirectResponse(url=f"/admin/automezzi/tag?error={urllib.parse.quote(err_msg)}", status_code=303)
            conn.execute(text("""
                INSERT INTO tag_automezzi (nome, colore, descrizione)
                VALUES (:nome, :colore, :desc)
    ...
    return RedirectResponse(url=f"/admin/automezzi/tag?success={urllib.parse.quote('Tag creato con successo.')}", status_code=303)
...
def edit_tag(id: int, r: Request, nome: str = Form(...), colore: str = Form("#0d6efd"), descrizione: str = Form(None)):
    ...
    try:
        with engine.begin() as conn:
            exists = conn.execute(text("""
                SELECT COUNT(*) FROM tag_automezzi 
    ...
    return RedirectResponse(url=f"/admin/automezzi/tag?success={urllib.parse.quote('Tag modificato con successo.')}", status_code=303)
...
def delete_tag(id: int, r: Request):
    ...
    return RedirectResponse(url=f"/admin/automezzi/tag?success={urllib.parse.quote('Tag eliminato con successo.')}", status_code=303)
...
def modifica_viaggio(
    ...
    with engine.begin() as conn:
        if role in ("admin", "global_fleet_manager"):
            v = conn.execute(text("SELECT * FROM viaggi_automezzi WHERE viaggio_id = :id"), {"id": id}).mappings().first()
        elif role == "fleet_manager":
            user_reparto_id = conn.execute(text("SELECT reparto_id FROM users WHERE user_id = :uid"), {"uid": uid}).scalar() or 0
            v = conn.execute(text("""
                SELECT v.*
        ...
        ora_partenza_clean = ora_partenza.strip()
        ...
        if ora_arrivo_clean and ora_arrivo_clean <= ora_partenza_clean:
            err_msg = urllib.parse.quote(f"L'orario di arrivo ({ora_arrivo_clean}) deve essere successivo all'orario di partenza ({ora_partenza_clean}).")
            sep = "&" if "?" in redirect_target else "?"
            return RedirectResponse(url=f"{redirect_target}{sep}error={err_msg}", status_code=303)
        ...
        if km_finali_val is not None:
            ...
            registra_storico_km(conn, aid, km_finali_val, "Modifica Viaggio", data_reg=v["data_viaggio"], user_id=uid, note=f"Modifica Viaggio #{id}")
