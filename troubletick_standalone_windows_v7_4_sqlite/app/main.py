...
def admin_servizi(r: Request, error: str = None):
    ...
    if isinstance(user, RedirectResponse):
        return user
    with engine.connect() as c:
        servizi = c.execute(text("SELECT s.servizio_id, s.descrizione, s.reparto_id, r.nome AS reparto_nome FROM servizi s LEFT JOIN reparti r ON s.reparto_id = r.reparto_id ORDER BY r.nome, s.descrizione")).mappings().all()
        reparti = c.execute(text("SELECT reparto_id, nome FROM reparti ORDER BY nome")).mappings().all()
...
def add_servizio(r: Request, descrizione: str = Form(...), reparto_id: int = Form(...)):
    user = require_superuser(r)
    if isinstance(user, RedirectResponse):
        return user
    with engine.begin() as c:
        c.execute(text("INSERT INTO servizi (descrizione, reparto_id) VALUES (:descrizione, :reparto_id)"), {"descrizione": descrizione, "reparto_id": reparto_id})
...
def edit_servizio_action(r: Request, servizio_id: int, descrizione: str = Form(...), reparto_id: int = Form(...)):
    user = require_superuser(r)
    if isinstance(user, RedirectResponse): return user
    with engine.begin() as c:
        c.execute(text("UPDATE servizi SET descrizione = :desc, reparto_id = :rid WHERE servizio_id = :id"),
                  {"desc": descrizione, "rid": reparto_id, "id": servizio_id})
    return RedirectResponse(url="/admin/servizi", status_code=303)
        ...
        with engine.begin() as c:
            ...
            for serv in data.get("servizi", []):
                ...
                if rep_id and not c.execute(text("SELECT servizio_id FROM servizi WHERE descrizione = :d AND reparto_id = :rid"), {"d": serv["descrizione"], "rid": rep_id}).scalar():
                    c.execute(text("INSERT INTO servizi (descrizione, reparto_id) VALUES (:d, :rid)"), {"d": serv["descrizione"], "rid": rep_id})
            # Materiali
...
def export_full(r: Request):
    ...
    with engine.connect() as c:
        ...
        servizi = c.execute(text("""
            SELECT s.descrizione, r.nome as reparto 
            FROM servizi s 
            JOIN reparti r ON s.reparto_id = r.reparto_id
        """)).mappings().all()
...
def download_import_esempio(r: Request):
    ...
    esempio = {
      ...
      "servizi": [
        {"descrizione": "Sostituzione Lampadine", "reparto": "Ufficio Tecnico"},
        {"descrizione": "Malfunzionamento Climatizzatori", "reparto": "Ufficio Tecnico"}
      ],
      "materiali": [
        ...
        with engine.begin() as c:
            for row in reader:
                if row.get("descrizione") and row.get("reparto_id"):
                    c.execute(text("""INSERT INTO servizi (descrizione, reparto_id) 
                        VALUES (:descrizione, :reparto_id)"""),
